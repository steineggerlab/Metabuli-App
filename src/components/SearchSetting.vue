      ...
      <v-card class="mb-3 search-settings-panel">
        ...
        <v-card-text>
          ...
          <v-sheet class="d-flex align-center mb-2">
            ...
            >
            <div class="ml-3">
              Selected Directory: {{ jobDetails.database }}
            </div>
          </v-sheet>
          ...
          <!-- BUTTONS -->
          <v-sheet class="d-flex align-center mb-2">
            <v-btn :loading="loading" @click="startJob" color="indigo">
              Run Metabuli
            ...
            <v-btn class="ml-3" @click="loadSampleData" color="indigo">
              Load Sample Data
            </v-btn>
          </v-sheet>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    async runBackend() {
      ...
      try {
        ...
        window.electron.onBackendCancelled((message) => {
          console.log("Backend cancelled:", message);
    ...
    async pollJobStatus(ticketid, interval = 500, timeout = 60000) {
